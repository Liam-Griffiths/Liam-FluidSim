export class CirclePhysics {
  ...
  checkCollisions(deltaTime) {
    ...
    for (const other of otherObjects) {
      if (other !== this.gameObject && other.circlePhysics) {
        const dx = this.gameObject.position.x - other.position.x;
        const dy = this.gameObject.position.y - other.position.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        const minDistance = this.radius + other.circlePhysics.radius;

        if (distance < minDistance) {
          // Collision response
          this.handleCollision(other, deltaTime);
        }
      }
    }
  }

  handleCollision(other, deltaTime) {
    ...
    // Calculate overlap
    const overlap = (this.radius + other.circlePhysics.radius - distance) / 2;
    ...
    const rvx = this.velocity.x - other.circlePhysics.velocity.x;
    const rvy = this.velocity.y - other.circlePhysics.velocity.y;
    ...
    const impulseScalar = (2 * velAlongNormal) / (this.mass + other.circlePhysics.mass);

    // Calculate new velocities based on mass
    this.velocity.x -= impulseScalar * other.circlePhysics.mass * nx;
    this.velocity.y -= impulseScalar * other.circlePhysics.mass * ny;
    other.circlePhysics.velocity.x += impulseScalar * this.mass * nx;
    other.circlePhysics.velocity.y += impulseScalar * this.mass * ny;
  }
